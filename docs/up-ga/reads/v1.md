...
<div class="tile-1" style="text-align:center; color:black">
  Total views on pages where the work can be read, as determined by Google Analytics. To count multiple reads on the same page by the same user, there must be a gap of at least 30 minutes from the last read of that page, even if that previous read was not counted.
</div>
